...
支持流程图、时序图、甘特图等 Mermaid 语法。优先改用注释语法图表、表格或任务列表表达；仅在没有合适图表类型时再使用 Mermaid。
...
5. **图表回复优先级**：默认先给 `chartType` 注释 + 表格示例；仅在找不到合适图表类型时再给 Mermaid。
6. **Mermaid 使用门槛**：只有当现有 `chartType` 无法满足表达需求、找不到合适图表类型时，才返回 Mermaid 代码块。
