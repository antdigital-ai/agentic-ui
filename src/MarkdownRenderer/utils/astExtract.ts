...
/**
 * 数字主体（含可选千分位 + 可选小数 + 可选科学计数法指数），不含正负号。
 * 用作百分比 / 紧凑后缀模式的内部子片段。
 *
 * 指数部分要求底数后跟非空指数（如 `1.2e3`、`1e-5`、`3E+10`），
 * 不接受 `1e`（指数缺位）、`e3`（缺底数）这类残缺写法。
 *
 * 注意：紧凑后缀字符集是 `[kKmMbBtT]`，与 `e/E` 不冲突。
 */
const NUMERIC_BODY_SOURCE = String.raw`\d+(?:[,，]\d+)*(?:\.\d+)?(?:[eE][+-]?\d+)?`;
