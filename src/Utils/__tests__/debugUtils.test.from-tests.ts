...
describe('debugUtils', () => {
  ...
  afterEach(() => {
    ...
    delete (global.window as Window & { __DEBUG_AGENTIC__?: number })
      .__DEBUG_AGENTIC__;
  });

  it('当 __DEBUG_AGENTIC__ 为 1 时应输出调试信息', () => {
    (
      global.window as Window & { __DEBUG_AGENTIC__?: number }
    ).__DEBUG_AGENTIC__ = 1;

    debugInfo('test message');

    expect(consoleLogSpy).toHaveBeenCalledTimes(1);
    expect(consoleLogSpy).toHaveBeenCalledWith('[Agentic Debug] test message');
  });
