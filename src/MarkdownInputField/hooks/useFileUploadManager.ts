 ...
 *
 * 覆盖 Office 三件套（excel/word/powerpoint）+ pdf + csv + txt + zip 等
 * 在移动端原生选择器里普遍能识别的 MIME 与扩展名集合。
 * 提取为模块级常量，避免每次 hook 调用时重新分配字符串。
 */
 ...
 * 历史位置 `src/MarkdownInputField/FileUploadManager/index.tsx` 现仅作为
...
export const useFileUploadManager = ({
   ...
   *
   * NOTE: 微信 / vivo|oppo / 移动端三个分支历史上分别由不同同事添加，
   * 当前都返回 `'*'`（让用户从系统文件浏览器自由选择）。这里保留三个分支
   * 以便日后差异化（例如微信里改成调起小程序选图）；如果未来彻底统一，
   * 可合并成 `if (isWeChat() || isVivoOrOppo() || isMobile()) return '*'`。
   */
