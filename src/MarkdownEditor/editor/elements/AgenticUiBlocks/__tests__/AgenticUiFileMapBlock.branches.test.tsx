...
vi.mock('../../../../../MarkdownInputField/FileMapView', () => ({
  FileMapView: ({ fileMap, className }: any) => (
    <div data-testid="map" data-size={fileMap.size} className={className} />
  ),
}));
  ...
  it('renders an empty normalized map and preserves hidden slate children', () => {
    render(
      <AgenticUiFileMapBlock
        attributes={{}}
        element={{ type: 'agentic-ui-filemap', value: '' } as any}
      >
        hidden
      </AgenticUiFileMapBlock>,
    );
    ...
    expect(
      screen.getByTestId('agentic-ui-filemap-hidden-children'),
        ...
        element={
          {
            ...
            value: {
              fileList: [{ uuid: 'u1', name: 'a.txt' }, { name: 'b.txt' }],
            },
          } as any
        }
      >
