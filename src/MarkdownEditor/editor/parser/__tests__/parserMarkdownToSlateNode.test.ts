...
describe('parserMarkdownToSlateNode', () => {
  describe('handleParagraph', () => {
    ...
    it('should keep single dollar inline math as plain text by default', () => {
      ...
      const textContent = collectText(paragraph);
    ...
    it('should keep numeric content wrapped in dollars as plain text', () => {
      ...
      expect(collectText(paragraph)).toContain('$100$');
    });
    ...
    it('should preserve inline <mark> color, background, and label attributes', () => {
      const markdown =
        'prefix <mark color="red" bg="#eee" label="Note">highlight</mark> suffix';
      const result = parserMarkdownToSlateNode(markdown);
      expect(result.schema).toHaveLength(1);
      expect(result.schema[0]).toMatchObject({
        type: 'paragraph',
