/**
 * Editor deepen12 safe：instance 空/变更、textArea enable、cut paste、
 * decorate table/card fallback、reset catch、WeChat native input 收尾。
 */
...
function makeClipboard(data: Record<string, string>, files: File[] = []) {
  return {
...
describe('Editor deepen12 safe residual branches', () => {
  beforeEach(() => {
    cleanup();
    editableProps = {};
  ...
  it('instance undefined；instance 变更触发 reset catch → deleteAll', async () => {
    setupStore();
  ...
  it('textAreaProps.enable mousedown 早退', () => {
    setupStore();
    render(
      <SlateMarkdownEditor
        prefixCls="ant-md"
  ...
  it('cut + files + markdown paste 分支', async () => {
    setupStore();
  ...
  it('decorate table fallback：findDom miss + focus.path table', () => {
    setupStore();
    vi.mocked(findByPathAndText).mockReturnValue([]);
  ...
  it('WeChat native input 收尾 composition', () => {
    setupStore();
