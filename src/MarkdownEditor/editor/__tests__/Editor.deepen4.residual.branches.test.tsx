/**
 * Editor deepen4：tableConfig reset 失败、WeChat pointer focus、
 * markdown/plain 空白 trim、decorate high 空与 comment refContent miss。
 */
...
let highlightReturn: any = [{ anchor: { path: [0, 0], offset: 0 } }];
...
import { EditorUtils, findByPathAndText, isPath } from '../utils/editorUtils';
...
function renderEditor(props: any = {}) {
  return render(
    <SlateMarkdownEditor
      prefixCls="ant-md"
      instance={props.instance ?? { id: 'inst4' }}
      initSchemaValue={
...
describe('Editor deepen4 residual branches', () => {
  beforeEach(() => {
    editableProps = {};
    highlightReturn = [{ anchor: { path: [0, 0], offset: 0 } }];
  ...
  it('tableConfig + initSchema：reset 抛错走 deleteAll', async () => {
    setupStore();
  ...
  it('WeChat pointer：contenteditable 内触发 focus', async () => {
    vi.mocked(isWeChat).mockReturnValue(true);
    const { editable } = setupStore();
    renderEditor({});
    await act(async () => {
    ...
    expect(ReactEditor.isFocused).toHaveBeenCalled();
  });

  it('paste：markdown / plain 仅空白 trim 早退', async () => {
    setupStore();
    renderEditor({});
    await act(async () => {
  ...
  it('decorate：high 空；comment refContent 未找到', async () => {
    highlightReturn = null;
    setupStore();
  ...
  it('WeChat native input：isComposing / composition end', async () => {
    vi.mocked(isWeChat).mockReturnValue(true);
    const { container } = setupStore({ inputComposition: true });
    renderEditor({});
    await act(async () => {
      const composing = new Event('input', { bubbles: true });
      Object.defineProperty(composing, 'isComposing', { value: true });
      container.dispatchEvent(composing);
      const ended = new Event('input', { bubbles: true });
      Object.defineProperty(ended, 'isComposing', { value: false });
      container.dispatchEvent(ended);
      await Promise.resolve();
    });
    expect(document.body).toBeTruthy();
  });
