...
import { keyArrow } from '../../../../editor/plugins/hotKeyCommands/arrow';
...
describe('arrow.ts', () => {
  ...
  describe('左箭头处理', () => {
    ...
    it('应该正确处理左箭头键 - 默认光标移动', () => {
      ...
      expect(mockEvent.preventDefault).toHaveBeenCalled();
      expect(mockEvent.stopPropagation).toHaveBeenCalled();
      expect(moveSpy).toHaveBeenCalledWith(mockEditor, {
        unit: 'offset',
        reverse: true,
      });
    });
  ...
  describe('右箭头处理', () => {
    it('应该正确处理右箭头键 - Editor.nodes 返回 media 时选中下一节点起点', () => {
      ...
      expect(mockEvent.preventDefault).toHaveBeenCalled();
      expect(selectSpy).toHaveBeenCalledWith(mockEditor, {
        path: [0, 2],
        offset: 0,
      });
    });
    ...
    it('应该正确处理右箭头键 - Editor.nodes 返回非 media/attach 时调用 moveAfterSpace', () => {
      ...
      expect(mockEvent.preventDefault).toHaveBeenCalled();
      expect(EditorUtils.moveAfterSpace).toHaveBeenCalledWith(
        mockEditor,
        [0, 1],
      );
    });
  ...
  describe('上箭头处理', () => {
    it('应该正确处理上箭头键 - media/attach元素的向上移动', () => {
      ...
      vi.spyOn(Editor, 'node').mockReturnValue([
        { type: 'media' },
        [0, 0],
      ] as any);
      const selectSpy = vi
