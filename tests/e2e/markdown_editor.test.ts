...
test('MarkdownEditor demo page should render correctly', async () => {
  ...
  try {
    ...
    const response = await page.goto('http://localhost:8000/~demos/docs-demos-markdowninputfield-custom-send-button', { 
      timeout: 10000, 
    ...
    if (response?.ok()) {
      ...
      console.warn('Could not connect to demo page. Make sure the dev server is running.');
    }
  } catch (error) {
