...
13. **四象限图 (quadrant)**
    ...
    将表格数据按「象限」列分组到 2×2 网格，常用于优先级矩阵、技术评估等场景。
    每行表格 = 一个条目，通过「象限」列的值自动归类到四个区域。

    | 字段          | 别名                                          | 必填 |
    | ------------- | --------------------------------------------- | ---- |
    | `name`        | `名称`、`标题`、`项目`、`name`、`title`       | 是   |
    | `quadrant`    | `象限`、`分类`、`类别`、`quadrant`、`category` | 是   |
    | `description` | `描述`、`简介`、`说明`                        | 否   |

    前 4 个不重复的象限值构成 2×2 网格的四个区域（按首次出现顺序排列）。
    不足 4 个象限时自动补空占位；超过 4 个象限值的行会被忽略。
    可通过 `fieldMap` 将语义字段显式映射到自定义表头。
    ...
    若注释中无法定位名称列或象限列，整表会降级为普通 Markdown 表格渲染。
