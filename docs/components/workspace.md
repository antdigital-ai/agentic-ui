...
<code src="../demos/workspace-file-tree-demo.tsx" description="树形展示文件，展开目录时通过 loadChildren 懒加载子节点">文件管理 - 文件树（懒加载）</code>
...
树形文件列表，目录节点在首次展开时通过 `loadChildren` 拉取子节点（懒加载）。

| 属性         | 说明                                                        | 类型                                                                | 默认值 | 版本 |
| ------------ | ----------------------------------------------------------- | ------------------------------------------------------------------- | ------ | ---- |
| treeData     | 根级树数据，目录懒加载时 `isLeaf: false` 且子级先为空或省略 | `FileTreeNode[]`                                                    | -      | -    |
| loadChildren | 展开无子项的目录时拉取子节点（可异步）                      | `(node: FileTreeNode) => FileTreeNode[] \| Promise<FileTreeNode[]>` | -      | -    |
...
| name     | 展示名称                                                           | `string`         | -      | -    |
| isLeaf   | 是否为文件；目录且懒加载时为 `false`，子级未加载时 `children` 为空 | `boolean`        | -      | -    |
| children | 已加载的子节点；懒加载前可省略或给 `[]`                            | `FileTreeNode[]` | -      | -    |
