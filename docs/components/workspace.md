...
<code src="../demos/workspace-file-list-tree-switch-demo.tsx" description="fileTreeSwitch 开启分段切换；受控 view 与外部按钮、面板内 Segmented 同步，平铺模式可搜索">文件管理 - 平铺与文件树动态切换</code>
...
| fileTreeSwitch      | 平铺列表与内嵌文件树切换（`view` / `onViewChange` 可受控）    | `FileTreeSwitchConfig`                                                                                          | -       | -      |
...
树形文件列表，目录节点在首次展开时通过 `onLoadChildren` 拉取子节点（懒加载）。
