...
- 🎨 **智能布局**: 根据文件数量自动调整展示布局
- 📱 **响应式设计**: 适配不同屏幕尺寸
- 🧩 **自定义点击**: 支持业务接管普通文件卡片点击，或禁用内置默认预览点击
- 🪪 **无 URL 卡片展示**: 无 `url` / `previewUrl` 但有 `type` 和 `name` 的文件会按类型展示文件图标和卡片
...
| 属性                      | 说明                                                                                                                           | 类型                                                             | 默认值       | 版本 |
| ------------------------- | ------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------ | ---- |
| `fileMap`                 | 文件映射对象                                                                                                                   | `Map<string, AttachmentFile>`                                    | -            | -    |
| `maxDisplayCount`         | 最多展示的非图片文件数量，传入则开启溢出控制并在超出时显示"查看所有文件"按钮                                                   | `number`                                                         | -            | -    |
| `showMoreButton`          | 是否显示"查看更多"按钮                                                                                                         | `boolean`                                                        | `false`      | -    |
| `itemRender`              | 自定义每个图片/视频条目的渲染，返回节点替换默认展示，常用于回显场景                                                            | `(file: AttachmentFile, defaultDom: ReactNode) => ReactNode`     | -            | -    |
| `onPreview`               | 预览文件回调。对图片：点击缩略图时触发，传入则阻止内置灯箱；对视频：传入则阻止内置弹窗；对普通文件：传入则阻止默认 window.open | `(file: AttachmentFile) => void`                                 | -            | -    |
| `onFileClick`             | 普通文件卡片点击回调。传入后优先于 `onPreview` 和默认 `window.open`，可由业务完全接管文件点击                                  | `(file: AttachmentFile) => void`                                 | -            | -    |
| `disableDefaultFileClick` | 禁用普通文件卡片点击触发的默认预览行为。未传 `onFileClick` 时生效，不影响悬浮操作区里的预览按钮                                | `boolean`                                                        | `false`      | -    |
| `onDownload`              | 下载文件回调                                                                                                                   | `(file: AttachmentFile) => void`                                 | -            | -    |
| `onViewAll`               | 点击"查看所有文件"回调，返回 `true` 时组件内部展开所有文件，返回 `false` 时由外部处理                                          | `(files: AttachmentFile[]) => boolean \| Promise<boolean>`       | -            | -    |
| `renderMoreAction`        | 自定义更多操作 DOM（传入则展示该 DOM，不传则不展示更多按钮）                                                                   | `(file: AttachmentFile) => React.ReactNode`                      | -            | -    |
| `customSlot`              | 自定义悬浮动作区 slot（传入则覆盖默认『预览/下载/更多』动作区）                                                                | `React.ReactNode \| ((file: AttachmentFile) => React.ReactNode)` | -            | -    |
| `style`                   | 自定义根容器样式（可覆盖布局，如 flexDirection、gap、wrap 等）                                                                 | `React.CSSProperties`                                            | -            | -    |
| `className`               | 自定义根容器类名                                                                                                               | `string`                                                         | -            | -    |
| `placement`               | 文件列表位置                                                                                                                   | `'left' \| 'right'`                                              | `` `left` `` | -    |
...
export default () => {
  ...
  return <FileMapView fileMap={fileMap} onPreview={handlePreview} />;
};
```
...
export default () => {
  const itemRender = (file: AttachmentFile, defaultDom: React.ReactNode) => (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      {defaultDom}
      <Button
        size="small"
        style={{ position: 'absolute', top: 4, right: 4 }}
        onClick={() => console.log('自定义操作', file)}
      >
        操作
      </Button>
    </div>
  ...
  return <FileMapView fileMap={fileMap} itemRender={itemRender} />;
};
```
...
export default () => {
  return (
    <FileMapView
      ...
      onFileClick={(file: AttachmentFile) => {
        message.info(`打开业务侧预览面板：${file.name}`);
      }}
...
- **视频文件**: 有 `url` 或 `previewUrl` 时使用视频缩略图展示
- **普通文件**: 使用列表布局展示文件图标、文件名和文件信息
- **无 URL 媒体文件**: 图片/视频没有 `url` 和 `previewUrl` 但有 `type/name` 时，降级为普通文件卡片展示
...
- 支持智能布局
- 支持响应式设计
- 支持自定义普通文件点击与禁用默认点击预览
- 支持无 URL 媒体文件按普通文件卡片降级展示
- 普通文件卡片补充基于文件名的 `aria-label`
