...
### 流式 Markdown：`streamingParagraphAnimation`

在 `markdownRenderConfig.renderMode` / `renderType` 为 `markdown` 且消息处于流式输出时，末段段落会使用淡入动画。**未传该字段时默认开启**。若需与旧版行为一致（关闭末段段落淡入），请显式传入 `false`：
...
interface MessageBubbleData {
  id: string; // 消息唯一标识
  role: 'user' | 'assistant' | 'system' | 'agent' | 'bot' | 'sessionNotice'; // 发送者角色
